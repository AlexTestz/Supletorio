...
#importar la clasificacion de texto
...
def main():
    ...
    st.sidebar.title("Información de Usuario")
    user_name = st.sidebar.text_input("Nombre de Usuario", "")
    edad = st.sidebar.text_input("Ingrese su edad", "")

    st.title("Base de datos vectorial")  # Establece el título de la aplicación
    st.header("Examen de desarrollo de sistemas")

    pdfs = st.file_uploader("Sube tus archivos PDF", type="pdf", accept_multiple_files=True)  # Crea un cargador de archivos para subir archivos PDF

    # Cuadro de texto para que el usuario ingrese cualquier texto
    user_input_text = st.text_area("Ingresa cualquier texto aquí:", "")

    # Botón para clasificar el texto ingresado
    if user_input_text:
        
        labels = ["política", "religión", "deporte", "otros"]
        # Clasificar el texto ingresado por el usuario con el modelo FinBERT
        ...
        classification = finbert_classifier(user_input_text)

        # Mostrar la clasificación
        st.write("Clasificación del texto ingresado:")
        st.write(classification)

        # Obtener la categoría principal
        main_category = classification[0]["label"]

        # Mostrar la categoría principal
        st.write("Categoría principal del texto:")
        st.write(main_category)

    if pdfs:
        for idx, pdf in enumerate(pdfs):  # Utilizamos enumerate para obtener el índice de iteración
            ...
            query = st.text_input(f'Escribe tu pregunta para el PDF {idx+1}...')  # Usamos idx+1 para mostrar el número de PDF

            # Botón para cancelar la pregunta
            cancel_button = st.button(f'Cancelar {idx+1}')  # Usamos idx+1 para generar una identificación única para cada botón
    ...
    st.write(f"tiempo de respuesta: {execution_time}segundos")
